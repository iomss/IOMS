    ...
    <el-row>
      <el-col>
        <div class="panel">
          <div class="header">
            <div class="tools">
              <el-button type="success" size="small" @click="FormVisible = true">新增备件名称</el-button>
          ...
          <div class="content">
            <el-table :data="tableData" stripe border style="width: 1500px" @selection-change="handleSelectionChange">
              ...
              <el-table-column prop="inLiability" label="供应商" />
            </el-table>
            <!-- 新增备件名称 -->
            <el-dialog title="新增备件名称" :visible.sync="FormVisible" :close-on-press-escape="false" :close-on-click-modal="false" width="1000px">
              <el-form ref="EditForm" :model="EditForm" :rules="FormRules" label-width="120px">
                <el-form-item label="备件名称" prop="name">
                  <el-input v-model="EditForm.name" placeholder="备件名称" size="small" />
                </el-form-item>
                <el-form-item label="备件类型" prop="name">
                  <el-select v-model="EditForm.systemId" filterable remote :remote-method="remoteMethodsystemId" :loading="loading" placeholder="备件类型" size="small" @focus="remoteMethodsystemId">
                    <el-option v-for="item in systemData" :key="item.id" :label="item.name" :value="item.id" />
                  </el-select>
                </el-form-item>
                <el-form-item label="品牌" prop="name">
                  <el-select v-model="EditForm.brandId" filterable remote :remote-method="remoteMethodbrandId" :loading="loading" placeholder="资产类别" size="small" @focus="remoteMethodbrandId" @change="changeBrand">
                    <el-option v-for="item in brandData" :key="item.id" :label="item.name" :value="item.id" />
                  </el-select>
                </el-form-item>
                <el-form-item label="型号" prop="name">
                  <el-select v-model="EditForm.modelId" filterable remote :remote-method="remoteMethodmodelId" :loading="loading" placeholder="资产类别" size="small" @focus="remoteMethodmodelId">
                    <el-option v-for="item in modelData" :key="item.id" :label="item.name" :value="item.id" />
                  </el-select>
                </el-form-item>
                <el-form-item label="单位" prop="name">
                  <el-input v-model="EditForm.name" placeholder="单位" size="small" />
                </el-form-item>
                <el-form-item label="供应商" prop="name">
                  <el-input v-model="EditForm.name" placeholder="供应商" size="small" />
                </el-form-item>
                <el-form-item label="录入人" prop="name">
                  <el-input v-model="EditForm.name" placeholder="录入人" size="small" />
                </el-form-item>
                <el-form-item label="录入日期" prop="enableTime">
                  <el-date-picker v-model="EditForm.enableTime" type="date" placeholder="录入日期" />
                </el-form-item>
                <el-form-item label="库存安全下限" prop="name">
                  <el-input v-model="EditForm.name" placeholder="库存安全下限" size="small" />
                </el-form-item>
                <el-form-item label="图片" class="form_mid">
                  <Uploadimg v-model="EditForm.resultImg" :reset="EditForm.resultImg" @uploadimg="uploadresultImg">aaa</Uploadimg>
                </el-form-item>
              </el-form>
