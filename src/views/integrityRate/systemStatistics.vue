<template>
  <div class="app-container">
    ...
    <el-form v-show="isShowSearch" :inline="true" :model="formData" class="demo-form-inline" size="small">
      ...
      <el-form-item label="一级系统">
        <el-select v-model="formData.systemId" placeholder="状态">
          <el-option label="全部" value="" />
          <el-option v-for="item in systemAllList" :key="item.id" :label="item.name" :value="item.id" />
        </el-select>
      </el-form-item>
...
export default {
  ...
  methods: {
    ...
    // 图表统计
    system() {
      this.$axios.get(`/api/OperationStatistic/GetSystemRate?positionId=${this.formData.positionId}&systemId=${this.formData.systemId}&beginTime=${this.formData.beginTime}&endTime=${this.formData.endTime}`).then(res => {
        this.dataAxis = res.map(item => { return item.name })
        this.dataAjax = res.map(item => { return (item.value * 100).toFixed(2) })
