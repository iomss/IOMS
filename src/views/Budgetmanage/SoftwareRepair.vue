            ...
            <el-table :data="tableData" stripe border style="width: 1200px" @selection-change="handleSelectionChange">
              ...
              <el-table-column prop="code" label="版本号">
                <template slot-scope="scope">
                  <el-button type="text">
                    <router-link :to="'/Budgetmanage/SoftwareRepairInfo/'+scope.row.id">{{ scope.row.code }}</router-link>
                  </el-button>
                </template>
              </el-table-column>
...
export default {
  ...
  methods: {
    ...
    create(row) { // 生成维护费用
      this.$axios.post('/api/UnitPrice', { type: 'SoftRepair' }).then(res => {
        this.$message.success('软件维修费用生成成功')
        this.getData()
      })
    },
    setvalid(row) { // 设置有效
    ...
    setvalidSubmit() {
      this.$axios.post('/api/UnitPrice/' + this.multipleSelection[0].id + '/SetValid').then(res => {
        this.$message.success('软件维修费用生效成功')
        this.getData()
